...
def distribute_cells_to_pages(cells, max_lines_per_column, num_columns=NUM_COLUMNS):
    """
    Distribue les cellules en tenant compte d'une ligne de séparation entre elles.
    Hauteur effective = sum(lignes_cellules) + (nb_cellules_colonne - 1).
    Garantit de retourner au moins une page vide.
    """
    if not cells:
        print(f"Avertissement (Max={max_lines_per_column}): Aucune cellule à distribuer. Retourne une page vide.")
        return [[[] for _ in range(num_columns)]] # Une page avec N colonnes vides

    pages_data = []
    current_page_columns = [[] for _ in range(num_columns)]
    current_col_lines = [0] * num_columns # Stocke la hauteur *effective* (lignes + séparateurs)
    current_col_cell_count = [0] * num_columns # Compte le nombre de cellules par colonne sur la page actuelle
    ...
    for i, cell in enumerate(cells):
        ...
        while search_idx < num_columns:
            is_first_cell_in_col = (current_col_cell_count[search_idx] == 0)

            # Calculer le coût d'ajout de cette cellule
            # Si c'est la première, le coût est juste ses lignes.
            # Sinon, c'est ses lignes + 1 (pour le séparateur au-dessus).
            cost_to_add = cell_lines
            if not is_first_cell_in_col:
                cost_to_add += 1 # Ajouter 1 pour la ligne de séparation

            # Vérifier si l'ajout dépasse la limite
            if current_col_lines[search_idx] + cost_to_add <= max_lines_per_column:
                # --- Placement possible ---
                # Avertissement si une cellule seule dépasse (rare avec la nouvelle logique mais possible)
                if is_first_cell_in_col and cell_lines > max_lines_per_column:
                ...
                # Mettre à jour la hauteur effective de la colonne
                current_col_lines[search_idx] += cost_to_add
                # Incrémenter le compteur de cellules pour cette colonne
                current_col_cell_count[search_idx] += 1
        ...
        if not found_placement:
            ...
            if any(current_col_cell_count): # Vérifier s'il y a des cellules plutôt que des lignes
                pages_data.append(current_page_columns)

            # Nouvelle page : Réinitialiser les compteurs
            current_page_columns = [[] for _ in range(num_columns)]
            current_col_lines = [0] * num_columns
            current_col_cell_count = [0] * num_columns
            col_index = 0 # Commencer à la première colonne

            # Placer la cellule dans la première colonne (col 0) de la nouvelle page
            # Le coût ici est juste cell_lines car c'est la première
            cost_on_new_page = cell_lines
            ...
            current_col_lines[0] += cost_on_new_page # Hauteur effective = lignes de la cellule
            current_col_cell_count[0] += 1

            # La prochaine cellule commencera sa recherche en fonction de la hauteur actuelle de la col 0
            if current_col_lines[0] >= max_lines_per_column:
                 col_index = 1
            else:
                 col_index = 0

        # Si on vient potentiellement de remplir la dernière colonne,
        # forcer une nouvelle page au prochain tour en réinitialisant les compteurs
        # (col_index sera >= num_columns si la colonne num_columns-1 a été remplie)
    ...
    # Ajouter la toute dernière page si elle contient quelque chose
    if any(current_col_cell_count):
        pages_data.append(current_page_columns)

    # S'assurer qu'au moins une page vide est retournée si pages_data est resté vide
    if not pages_data:
